...
def _writeOETSideBySideChapterBook( level:int, folder:Path, rvBible, lvBible, state:State, BBB:str ) -> tuple[bool,str,list[str]]:
    """
    Write all the OET side-by-side chapter pages (plus the per-book index page) for one book.

    This is the per-book body of createOETSideBySideChapterPages.
...
{makeBottom( level, rvBible.abbreviation, 'chapter' )}'''
    ...
    assert rvBible.getNumVerses( BBB, '-1' ) or BBB in ('INT',) # OET always has intro for its books
...
<h2><a title="View just the Literal Version by itself" href="{'../'*level}OET-LV/byC/{BBB}_Intro.htm#Top">Literal Version</a></h2>''' \
            if c==-1 else f'''<h1 id="Top">Open English Translation {ourTidyBBBwithNotes} Chapter {c}</h1>
{cNav}
{state.OET_UNFINISHED_WARNING_HTML_PARAGRAPH}
{f'{state.JAMES_NAME_NOTE_HTML_PARAGRAPH}{NEWLINE}' if BBB=='JAM' else ''}<div class="RVLVcontainer">
...
<h2><a title="View just the Literal Version by itself" href="{'../'*level}OET-LV/byC/{BBB}_C{c}.htm#Top">Literal Version</a> <button type="button" id="marksButton" title="Hide/Show underline and strike-throughs" onclick="hide_show_marks()">Hide marks</button></h2>'''
            ...
            rvHtml = do_OET_RV_HTMLcustomisations( f'OET-RV1 chapter={BBB}_{c}', convertVerseEntryListToHtml( level, 'OET-RV', (BBB,str(c)), 'chapter', rvContextList, rvVerseEntryList, basicOnly=False, state=state, livenWordLinks=True, colouriseWordClasses=False ) )
            lvHtml = do_OET_LV_HTMLcustomisations( f'OET-LV1 chapter={BBB}_{c}', convertVerseEntryListToHtml( level, 'OET-LV', (BBB,str(c)), 'chapter', lvContextList, lvVerseEntryList, basicOnly=False, state=state, livenWordLinks=True, colouriseWordClasses=False ) )
            ...
            if c < 1:
                ...
                for n,rvSectionHtml in enumerate( rvSections ):
                    ...
                    if lvRest:
                        ...
                        # Handle nb paragraphs
                        nbStartCount, nbEndCount = lvChunk.count('<p class="nb"'), lvChunk.count('</p><!--nb-->')
                        if nbStartCount == nbEndCount+1:
                            # print( f"OET-LV {BBB} {c} chunk #{len(lvChunks)+1} {nbStartCount=} {nbEndCount=}\n{lvChunk=}")
                            lvChunk = f'{lvChunk}</p><!--nb-->'
                        elif nbStartCount+1 == nbEndCount:
                            # print( f"OET-LV {BBB} {c} chunk #{len(lvChunks)+1} {nbStartCount=} {nbEndCount=}\n{lvChunk=}")
                            lvChunk = f'<p class="nb">{lvChunk}'
                        else: assert nbStartCount == nbEndCount == 0
                        # print( f"\nOET-LV {BBB} {c} chunk #{len(lvChunks)+1} was {nbStartCount=} {nbEndCount=}\n{lvChunk=}\n{lvChunk=}" )
                        assert checkHtml( f'OET-LV {BBB}_C{c} chunk #{len(lvChunks)+1}', lvChunk, segmentOnly=True ), f"OET-LV {BBB} {c}\n{lvChunk=}"
                ...
                for rvSection,lvChunk in zip( rvSections, lvChunks, strict=True ):
                    ...
                    assert checkHtml( f'OET-RV {BBB}_C{c} section', rvSection, segmentOnly=True )
                    assert checkHtml( f'OET-LV {BBB}_C{c} chunk', lvChunk, segmentOnly=True ), f"OET-LV {BBB} {c}\n{lvChunk=}"
...
#             assert BBB in ('INT',)
#             print( f"CAN'T DO '{BBB}' CHAPTER PAGES" ); halt
...
#             except TypeError:
#                 chapterHtml = f'''{chapterHtml}<p class="Note">OET-LV {BBB} didn't have any introduction.</p>''' # It didn't exist
...
def _createOETSideBySideChapterBook_MP( parameters ) -> tuple[bool,str,list[str]]:
    """
    ...
    Returns the (True, BBB, filenames) result from _writeOETSideBySideChapterBook.
    """
    level, folder, BBB = parameters
    rvBible = state.preloadedBibles['OET-RV']
    lvBible = state.preloadedBibles['OET-LV']
    resultBool, BBBResult, filenames = _writeOETSideBySideChapterBook( level, folder, rvBible, lvBible, state, BBB )
    assert resultBool is True
    return ( resultBool, BBBResult, filenames )
# end of createChapterPages.py._createOETSideBySideChapterBook_MP
...
def createOETSideBySideChapterPages( level:int, folder:Path, rvBible, lvBible, state:State ) -> list[str]:
    ...
    # Now create the actual OET chapter pages (one forked worker per book)
    BBBs, filenames = [], []
    mpBookParameters = [] # (level, folder, BBB) tuples for the forked workers
    for BBB in state.BBBsToProcess['OET']:
        if BibleOrgSysGlobals.maxProcesses > 1 \
        and not BibleOrgSysGlobals.alreadyMultiprocessing: # Use multiprocessing for these OET chapter pages
            mpBookParameters.append( (level, folder, BBB) )
        else: # no multiprocessing available -- do this book sequentially
            resultBool, BBBResult, bookFilenames = _writeOETSideBySideChapterBook( level, folder, rvBible, lvBible, state, BBB )
            assert resultBool is True
            if bookFilenames:
...
{cNav}{f'{NEWLINE}{state.JAMES_NAME_NOTE_HTML_PARAGRAPH}' if 'OET' in thisBible.abbreviation and BBB=='JAM' else ''}{f'{NEWLINE}{state.RV_ONLY_WARNING_HTML_PARAGRAPH}' if thisBible.abbreviation=='OET-RV' else f'{NEWLINE}{state.LV_ONLY_WARNING_HTML_PARAGRAPH}' if thisBible.abbreviation=='OET-LV' else ''}{f'{state.BLACK_LETTER_FONT_HTML_PARAGRAPH}{NEWLINE}' if thisBible.abbreviation=='KJB-1611' else ''}'''
    ...
    <button type="button" id="marksButton" title="Hide/Show underline and strike-throughs" onclick="hide_show_marks()">Hide marks</button>
                ...
                textHtml = convertVerseEntryListToHtml( level, thisBible.abbreviation, (BBB,str(c)), 'chapter', contextList, verseEntryList, basicOnly=False, state=state, livenWordLinks=isinstance( thisBible, ESFMBible.ESFMBible ) or thisBible.abbreviation in ('BSB','MSB'), colouriseWordClasses=False, addNoLinkYetSpans=False )
...
{makeBottom( level, thisBible.abbreviation, 'chapter' )}'''
            ...
            filenames.append( filename )
            filepath = folder.joinpath( filename )
            # BBBLinks.append( f'<a title="{bos_books_codes_py.get_english_name_nr(BBB)}" href="{filename}#Top">{ourTidyBBB}</a>' )
...
{makeBottom( level, thisBible.abbreviation, 'chapter' )}'''
    ...
    indexHtml = f'''{top}
<h2>Index of books</h2>
{f'<a title="Go to OET main site" href="https://OpenEnglishTranslation.Bible"><img class="OETWideLogo" src="{'../'*level}oet-logo-wide.png" alt="OET wide logo"></a>\n' if 'OET' in thisBible.abbreviation else ''}<h1 id="Top">{thisBible.abbreviation} chapter pages</h1>{f'{NEWLINE}{state.JAMES_NAME_NOTE_HTML_PARAGRAPH}' if 'OET' in thisBible.abbreviation and BBB=='JAM' else ''}{f'{NEWLINE}{state.RV_ONLY_WARNING_HTML_PARAGRAPH}' if thisBible.abbreviation=='OET-RV' else f'{NEWLINE}{state.LV_ONLY_WARNING_HTML_PARAGRAPH}' if thisBible.abbreviation=='OET-LV' else ''}{f'{NEWLINE}{state.BLACK_LETTER_FONT_HTML_PARAGRAPH}' if thisBible.abbreviation=='KJB-1611' else ''}
{navBookListParagraph}
