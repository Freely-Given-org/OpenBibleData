...
CHANGELOG:
    ...
    2024-04-30 In state.TEST_MODE, only make parallel verse pages for the actual state.TEST_BOOK_LIST
...
def createParallelVersePages( level:int, folder:Path, state:State ) -> bool:
    ...
    try:
        ...
        if state.UPDATE_ACTUAL_SITE_WHEN_BUILT and not state.TEST_MODE: # Ignore missing versions in test modes
            raise e
    ...
    top = makeTop( level, None, 'parallelVerse', None, state ) \
            .replace( '__TITLE__', f"Parallel Verse View{' TEST' if state.TEST_MODE else ''}" ) \
...
def createParallelVersePagesForBook( level:int, folder:Path, BBB:str, BBBLinks:list[str], parallelVersions:list[str], state:State ) -> bool:
    ...
    if numChapters >= 1:
        ...
        for c in range( -1, numChapters+1 ):
            ...
            vPrint( 'Info', DEBUGGING_THIS_MODULE, f"      Creating {'TEST ' if state.TEST_MODE else ''}parallel pages for {BBB} {C}…" )
                    ...
                    if versionAbbreviation in state.selectedVersesOnlyVersions: # then thisBible is NOT a Bible object, but a dict
                        ...
                        try:
                            ...
                            if versionAbbreviation == 'OET-RV':
                                ...
                                if state.DO_SPELL_CHECKS and parRef not in ('JOB_24:1','PSA_8:5','EZE_-1:0',): # TODO Check these out
                                ...
                                if state.DO_SPELL_CHECKS and parRef not in ('MAT_11:21','LUK_10:13','ACT_7:2','ACT_7:4'): # TODO Check these out
                                    textHtml = spellCheckAndMarkHTMLText( versionAbbreviation, parRef, textHtml, textHtml, state ) # Puts spans around mispellings
                                ...
                                textHtml = do_LSV_HTMLcustomisations( f'ParallelVerseTxt={parRef}', textHtml )
                                if state.DO_SPELL_CHECKS:
                                    textHtml = spellCheckAndMarkHTMLText( versionAbbreviation, parRef, textHtml, textHtml, state ) # Puts spans around mispellings
                            elif versionAbbreviation == 'T4T':
                                textHtml = do_T4T_HTMLcustomisations( f'ParallelVerseTxt={parRef}', textHtml )
                                if state.DO_SPELL_CHECKS:
                                    textHtml = spellCheckAndMarkHTMLText( versionAbbreviation, parRef, textHtml, textHtml, state ) # Puts spans around mispellings
                                ...
                                if versionAbbreviation in ('KJB-1611','Bshps','Gnva','Cvdl','TNT','Wycl'):
                                    modernisedTextHtml = modernisedTextHtml.replace( 'J', 'Y' ).replace( 'Ie', 'Ye' ).replace( 'Io', 'Yo' ) \
                                                                    .replace( 'Yewel', 'Jewel' ).replace( 'Yudge', 'Judge' ).replace( 'Yuniper', 'Juniper' ).replace( 'Yust', 'Just' ).replace( 'KYB', 'KJB' ) # Fix overreaches
...
<br>   ({SRtranscription.replace('<br>','<br>   ')})'''
                            ...
                            elif versionAbbreviation in ('UGNT','SBL-GNT','RP-GNT','TC-GNT','BrLXX'):
                                ...
                                if C!='-1' and V!='0' and textHtml:
                                    ...
                                    # assert checkHtml( f'brightenedUHB1 {parRef}', textHtml, segmentOnly=True )
...
<br>   ({uhbTranscription.replace('<br>','<br>   ')})'''
                            ...
                            if textHtml:
                                assert checkHtml( f'Parallel textHtml {versionAbbreviation} {parRef}', textHtml, segmentOnly=True )
                                ...
                                if versionAbbreviation in ('UGNT','SBL-GNT','RP-GNT','TC-GNT') \
                            ...
                            vHtml = f'''<p id="OET-RV" class="parallelVerse"><span id="C{C}V{V}" class="wrkName"><a id="C{C}" title="{state.BibleNames['OET']}" href="{'../'*BBBLevel}OET/bySec/{BBB}_S{sectionNumber}.htm#V{V}">OET</a> <small>(<a id="V{V}" title="View {state.BibleNames['OET-RV']} section (by itself)" href="{'../'*BBBLevel}OET-RV/bySec/{BBB}_S{sectionNumber}.htm#V{V}">OET-RV</a>)</small></span> <span class="noVerse"><small>{warningText}</small></span></p>'''
                            # else:
                    ...
                    if vHtml:
                        ...
                        assert checkHtml( f'Parallel vHtml {versionAbbreviation} {parRef}', vHtml, segmentOnly=True )
                ...
                if state.UPDATE_ACTUAL_SITE_WHEN_BUILT and not state.TEST_MODE and not state.TEST_VERSIONS_ONLY:
...
<br>  <small>See the Scriptura Psalm Layer-by-Layer <a href="https://psalms.scriptura.org/w/Psalm_{C}_Verse-by-Verse#v._{V}">analysis for this verse</a> (but that link requires making an account there)</small>.'''
                ...
                top = makeTop( BBBLevel, None, 'parallelVerse', None, state ) \
                        .replace( '__TITLE__', f"{ourTidyBBB} {C}:{V} Parallel Verse View{' TEST' if state.TEST_MODE else ''}" ) \
...
{makeBottom( BBBLevel, 'parallelVerse', state )}'''
    ...
    top = makeTop( BBBLevel, None, 'parallelVerse', None, state) \
            .replace( '__TITLE__', f"{ourTidyBBB} Parallel Verse View{' TEST' if state.TEST_MODE else ''}" ) \
            .replace( '__KEYWORDS__', 'Bible, parallel, verse, view, display, index' )
...
def brightenUHB( BBB:str, C:str, V:str, brightenUHBTextHtml:str, verseEntryList, state:State ) -> tuple[str,list[str]]:
    ...
    assert checkHtml( f'adjustedBrightenUHBTextHtml {UHBRef}', adjustedBrightenUHBTextHtml, segmentOnly=True ) # TODO: REMOVE This should be irrelevant/unnecessary
    ...
    assert checkHtml( f'Middle of brightenedUHB {UHBRef}', brightenUHBTextHtml, segmentOnly=True )
    classKeySet = set()
    if allExtras:
        # Find each word in brightenTextHtml, find the word info in allExtras, and then update brightenTextHtml with more classes
        ...
        for _safetyCount1 in range( len(strippedHebWords)+1 ):
            ...
            assert checkHtml( f'Extras loop bottomA for brightenedUHB {UHBRef} {_safetyCount1=}', brightenUHBTextHtml, segmentOnly=True )
            verseWordNumberIndex += 1
            if verseWordNumberIndex >= len(strippedHebWords):
                break
            ...
            # print( f"              {searchHtml=}" )
            assert checkHtml( f'Extras loop bottomB for brightenedUHB {UHBRef} {_safetyCount1=}', brightenUHBTextHtml, segmentOnly=True )
    ...
    assert checkHtml( f'Finished brightenedUHB {UHBRef}', brightenUHBTextHtml, segmentOnly=True )
    ...
    for verseEntry in thisVerseEntryList:
        ...
        if verseEntry.getMarker() in ('v~','p~'):
            for word in verseEntry.getCleanText().split():
                ...
                and '/(' not in word \
                and word not in (
                    'A','An', 'And','Again', 'All', 'Allow', 'Also', 'Am', 'About', 'Accept', 'Acquit', 'Actually', 'Add', 'Adorn', 'Adulteries', 'Adultresses', 'Afflict', 'After', 'Against', 'Alas', 'Alleging', 'Already',
                        'Although', 'Altogether', 'Always', 'Among', 'Ancestors', 'Answered', 'Antiquity', 'Apart', 'Appoint', 'Approach', 'Arise', 'Arms', 'Arrogant', 'Arrows', 'Asia', 'Asian', 'Assemble', 'Assuredly', 'Attack',
                    ...
                    'Fear', 'Feet', 'Fine', 'Fir', 'Five', 'Foolishness', 'Fools', 'For', 'For/Because', 'Four', 'Friend', 'From',
                        'Face', 'Faces', 'Faithful', 'Fall', 'Falsehood', 'Far', 'Favor', 'Fell', 'Fellow-imitators', 'Fifty', 'Fill', 'Filled', 'Finally', 'Fire', 'First', 'Flash', 'Flee', 'Flies', 'Flocks', 'Food', 'Foolish',
                         'Foot', 'Forbid', 'Forget', 'Forgets', 'Forgive', 'Formless', 'Forty', 'Found', 'Foxes', 'Freedmen', 'Fruit', 'Fulfill', 'Furthermore', 'Futility',
                    'Gather', 'Girded', 'Give', 'Go','Good', 'Goodness',
                        'Grace', 'Gracious', 'Grain', 'Grass', 'Great', 'Greater', 'Greatly', 'Greet', 'Greetings', 'Groaning', 'Grow',
                        'Guard', 'Guards', 'Guilt', 'Gave', 'Generation', 'Generously', 'Glorious', 'Glory', 'Going',
                    'Hand', 'Hands', 'Harm', 'Has', 'Hatred', 'Haughtiness', 'Healing', 'Her/its', 'Here', 'His/its', 'House', 'Honey', 'Honour', 'Honoured', 'Hope',
                        'Ha', 'Had', 'Half', 'Half-hearted', 'Happy', 'Harbours', 'Harness', 'Hasten', 'Hate', 'Have', 'Hear', 'Heard', 'Heart', 'Hebrew', 'Help', 'Hide', 'Hiding', 'Hill', 'Him', 'Himself',
                        'His', 'Hollow', 'Honor', 'Honored', 'Horses', 'Houses', 'However', 'Humankind', 'Hungry', 'Hunt', 'Hurry', 'Husbands', 'Hush', 'Hypocrite', 'Hypocrites',
                    'If/because', 'In/on/at/with', 'Incline', 'Iniquities', 'Insight', 'Iron', 'Is', 'It',
                        'Idols', 'Imitators', 'Immediately', 'In/on', 'Indeed', 'Inheritance', 'Inoffensive', 'Instead',
                        'Instruct', 'Instruction', 'Integrity', 'Interpret', 'Into', 'Iota', 'Israelite',
                    'Joy', 'Just',
                    'Keep',
                    ...
                    'Say','See','Satan','Sacrifices', 'Salt', 'Scale[s]', 'Seek', 'Send', 'Seven', 'She',
                        ...
                        'Sackcloth', 'Sacrifice', 'Said', 'Sanctify', 'Sat', 'Satisfied', 'Satisfy', 'Save', 'Saw', 'Says', 'Scatter', 'Scorn', 'Sea', 'Seal', 'Search',
                        'Seed', 'Seeing', 'Seen', 'Sell', 'Separate', 'Serpents', 'Servant', 'Servants', 'Serve', 'Set', 'Sevens', 'Severely',
                        'Shake', 'Sharp', 'Shepherd', 'Shepherds', 'Shoots', 'Shoulder', 'Shoulder-pieces', 'Shout', 'Show',
                    'THE', 'Take', 'Taken', 'Teacher', 'Tell', 'Ten', 'Testing', 'There', 'Thereafter', 'Therefore', 'These', 'They', 'This',
                        'Thorns', 'Three', 'Throw', 'Two',
                        'To/for', 'To/from', 'Toil(s)', 'Traders', 'Train', 'Treasure', 'Tremble', 'Trembling', 'Truth', 'Turbans', 'Twelve',
