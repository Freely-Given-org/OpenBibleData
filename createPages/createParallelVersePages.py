                    ...
                    if versionAbbreviation in state.selectedVersesOnlyVersions: # then thisBible is NOT a Bible object, but a dict
                        ...
                        try:
                            ...
                            if versionAbbreviation == 'OET-RV':
                                # if BBB=='MRK' and C=='3' and V=='9': print( f"AAA {parRef} {versionAbbreviation} {textHtml=}" )
                                ...
                                if footnoteFreeTextHtml == footnoteFreeTextHtmlWEB:
                                ...
                                if adjustedForeignTextHtml != textHtml: # only show it if it changed
                                    ...
                                    if '<div ' in textHtml: # it might have had footnotes in a <div>, but we want the transliteration BEFORE the footnotes
                                        ...
                                        textHtml = textHtml.replace( '<hr', f'<span class="{versionAbbreviation}_trans"><br>  ({adjustedForeignTextHtml})</span><hr' ) \
                                                        if '<hr ' in textHtml else \
                                                    f'{textHtml}<span class="{versionAbbreviation}_trans"></span><br>  ({adjustedForeignTextHtml})'
                                    else: # no <div>s so should be ok to add a span
                                        assert '</div>' not in textHtml
                                        textHtml = f'{textHtml}<span class="{versionAbbreviation}_trans"><br>  ({adjustedForeignTextHtml})</span>'
                            elif versionAbbreviation == 'SR-GNT':
                                if C!='-1' and V!='0' and textHtml:
                                    # print( f"{parRef} SR-GNT {verseEntryList=} {textHtml=} {transcription=}" )
                                ...
                                transcription = transliterate_Greek(footnoteFreeTextHtml) # Colourisation and nomina sacra gets carried through
                                ...
                                or 'Uh' in transcription or ' uh' in transcription or transcription.startswith('uh'):
                                    raise ValueError( f"Bad Greek transcription for {versionAbbreviation} {parRef} {transcription=} from '{textHtml}'" )
...
<br>   ({transcription})</span>{keysHtml}'''

                            elif versionAbbreviation in ('UGNT','SBL-GNT','TC-GNT','BrLXX'):
                                transcription = transliterate_Greek(footnoteFreeTextHtml)
                                if 'Ah' in transcription or ' ah' in transcription or transcription.startswith('ah') \
                                or 'Eh' in transcription or ' eh' in transcription or transcription.startswith('eh') \
                                or 'Oh' in transcription or ' oh' in transcription or transcription.startswith('oh') \
                                or 'Uh' in transcription or ' uh' in transcription or transcription.startswith('uh'):
                                ...
                                if transcription:
                                    ...
                                    if '<div ' in textHtml: # it might have had footnotes in a <div>, but we want the transliteration BEFORE the footnotes
                                        assert '</div>' in textHtml
                                        textHtml = textHtml.replace( '<hr', f'<span class="{versionAbbreviation}_trans"><br>  ({transcription})</span><hr' ) \
                                                        if '<hr ' in textHtml else \
                                            f'{textHtml}<span class="{versionAbbreviation}_trans"></span><br>  ({transcription})'
                                    else: # no <div>s so should be ok to add a span
                                        assert '</div>' not in textHtml
                                ...
                                uhbTranscription = transliterate_Hebrew(footnoteFreeTextHtml).replace( 'ə', '<small>ə</small>' )
...
<br>   ({uhbTranscription})</span>{keysHtml}'''
                            ...
                            if textHtml:
                                ...
                                and 'SR-GNT' in greekWords and greekWords[versionAbbreviation]!=greekWords['SR-GNT']:
                                    ...
                                    if '<div ' in textHtml: # it might be a book intro or footnotes -- we can't put a <div> INSIDE a <p>, so we append it instead
                                        assert '</div>' in textHtml
                                        vHtml = f'''<p id="{versionAbbreviation}" class="parallelVerse"><span class="{spanClassName}"><a title="View {state.BibleNames[versionAbbreviation]} {'details' if versionAbbreviation in state.versionsWithoutTheirOwnPages else 'chapter'}" href="{versionNameLink}">{versionAbbreviation}</a></span> {textHtml.replace('<hr','</p><hr')}'''
                                    ...
                                    if '<div ' in textHtml: # it might be a book intro or footnotes -- we can't put a <div> INSIDE a <p>, so we append it instead
                                        assert '</div>' in textHtml
                                        vHtml = f'''<p id="{versionAbbreviation}" class="parallelVerse"><span id="C{C}V{V}" class="wrkName"><a id="C{C}" title="View {state.BibleNames['OET']} chapter (side-by-side versions)" href="{'../'*BBBLevel}OET/byC/{BBB}_C{C}.htm#Top">OET</a> <small>(<a id="V{V}" title="View {state.BibleNames['OET-RV']} chapter (by itself)" href="{'../'*BBBLevel}OET-RV/byC/{BBB}_C{C}.htm#Top">OET-RV</a>)</small></span> {textHtml.replace('<hr','</p><hr')}'''                                    
                                    else: # no <div>s so should be ok to put inside a paragraph
                                        assert '</div>' not in textHtml
                                    ...
                                    versionNameLink = f'''{'../'*BBBLevel}{versionAbbreviation}/details.htm#Top''' if versionAbbreviation in state.versionsWithoutTheirOwnPages else f'''{'../'*BBBLevel}{versionAbbreviation}/byC/{BBB}_C{C}.htm#Top'''
                                    if '<div ' in textHtml: # it might be a book intro or footnotes
                                        assert '</div>' in textHtml
                                        vHtml = f'''<p id="{versionAbbreviation}" class="parallelVerse"><span class="wrkName"><a title="View {state.BibleNames[versionAbbreviation]} {'details' if versionAbbreviation in state.versionsWithoutTheirOwnPages else 'chapter'}" href="{versionNameLink}">{versionAbbreviation}</a></span> {textHtml.replace('<hr','</p><hr')}'''
                                    else: # no <div>s so should be ok to put inside a paragraph
                                        assert '</div>' not in textHtml
                ...
                bmmHtml = getBibleMapperMaps( BBBLevel, BBB, C, V, state.preloadedBibles['OET-RV'] )
                if bmmHtml:
                    bmmHtml = f'''<div id="BMM" class="parallelBMM"><a title="Go to BMM copyright page" href="{'../'*BBBLevel}BMM/details.htm#Top">BMM</a> <b>BibleMapper.com Maps</b>: {bmmHtml}</div><!--end of BMM-->'''
                    parallelHtml = f'{parallelHtml}\n<hr style="width:40%;margin-left:0;margin-top: 0.3em">\n{bmmHtml}'
...
def brightenSRGNT( BBB:str, C:str, V:str, brightenTextHtml:str, verseEntryList, state:State ) -> Tuple[str,List[str]]:
    ...
    punctuatedGrkWords = ( brightenTextHtml.replace( '¶', ' ' ).replace( '⇔', ' ' ).replace( '§', ' ' )
                                        #  .replace( '’ ”', '’”' ).replace( '” ’', '”’' ).replace( NARROW_NON_BREAK_SPACE, ' ' ) # Leave NNBSP
                                         .replace( NON_BREAK_SPACE, ' ' ).replace( EN_SPACE, ' ' ).replace( EM_SPACE, ' ' )
                                         .replace( '<br>', ' ' ).replace( '   ', ' ' ).replace( '  ', ' ' )
                                         .split( ' ' )
                        )
    strippedGrkWords = [punctuatedGrkWord.lstrip( '“‘˚(' ).rstrip( '.,?!:”’ ·;)–…' ) for punctuatedGrkWord in punctuatedGrkWords] # Includes NNBSP
    ...
    for strippedGrkWord in strippedGrkWords:
        assert strippedGrkWord, f"{brRef} {strippedGrkWords=}"
    ...
    if allExtras:
        ...
        for _safetyCount1 in range( len(punctuatedGrkWords)+1 ):
            ...
            # print( f"{brRef} {rawGrkWord=} from {punctuatedGrkWords=} from {brightenTextHtml=}" )
...
def brightenUHB( BBB:str, C:str, V:str, brightenUHBTextHtml:str, verseEntryList, state:State ) -> Tuple[str,List[str]]:
    ...
    if allExtras:
        ...
        for _safetyCount1 in range( len(strippedHebWords)+1 ):
            checkHtml( f'Extras loop of brightenedUHB {UHBRef} {_safetyCount1=}', brightenUHBTextHtml, segmentOnly=True )
            rawHebWord = strippedHebWords[verseWordNumberIndex]
            # print( f"Start of {UHBRef} loop1 {_safetyCount1=} {verseWordNumberIndex=} {rawHebWord=}" )
...
ENGLISH_WORD_MAP = ( # Place longer words first,
    ...
    ((' abideth',),' abides'),((' abydinge',),' abiding'), ((' abyde ',),' abide '),
        ...
        ((' accorde ',' acorde '),' accord '),((' acordynge',),' according'), (('knoulechide',),'acknowledged'),
        ...
        (('Andrewe',),'Andrew'),
            ...
            (('answerden','answerede','answerde','answeride','aunswered'),'answered'),((' aunswere ',' answere '),' answer '),((' aunswere,',),' answer,'),((' aunswere:',' answere:'),' answer:'),
    ...
    ((' backe ',),' back '), (('baptysed','baptisid'),'baptised'), (('baptisynge','baptisyng'),'baptising'), (('baptisme','baptyme','baptym'),'baptism'), ((' baptyse',),' baptise'),
        ...
        ((' braunches',),' branches'),((' braunch',' braunche'),' branch'),
            ((' breake ',),' break '), ((' brest ',),' breast/chest '), (('britheren',),'brethren/brothers'),(('brithre.',),'brethren/brothers.'),(('brethren:','brethre:'),'brethren/brothers:'),
            ...
            ((' brookes',' brokes'),' brooks'), ((' brouyten ',' brouyte '),' brought '),
    ...
    (('Cæsar','Cesar'),'Caesar'),
        ...
        ((' coostis',' coastes'),' coasts'), ((' cootis',' coottes',' coates',' cotes'),' coats'),
            ...
            ((' cometh',' commeth'),' cometh/comes'),
                ...
                (('companye',),'company'), (('complaynte',),'complaint'), (('comprehendiden',),'comprehended'),
            (('conseyue ',),'conceive '),
    ...
    ((' dayly',' daylie'),' daily'),
        ...
        ((' dekenes',),' deacons'), ((' deed',),' dead'), (('Deare ',),'Dear '),((' deare ',' dere '),' dear '), ((' deeth',' deth',' derth'),' death'),
            ...
            ((' degre ',),' degree '),
            ((' delyte',),' delight'), (('delyueraunce','delyuerauce','deliueraunce','deliuerance'),'deliverance'),((' deliuered',' delyuered'),' delivered'),((' delyuer ',' deliuer '),' deliver '),
            ...
            (('descendinge',),'descending'),(('descende ',),'descend '),
                ...
                ((' distriede',' destroied'),' destroyed'),((' distrie ',' destroye ',' distroye '),' destroy '),
        ...
        (('dredden','dredde'),'dreaded'), ((' dryncke',' drynke',' drynk',' drinke'),' drink'), ((' driueth',' driveth'),' drives'), ((' driue',' dryue'),' drive'),
    ...
    ((' failinge',),' failing'), ((' fayle ',),' fail '), ((' faynte ',' faynt '),' faint '), ((' feith',' fayth'),' faith'),
        ...
        ((' fielde',' feeld',' felde'),' field'), ((' feendis',),' fiends'),
            ...
            ((' filleth ',' fylleth '),' filleth/fills '),((' fillid ',),' filled '),((' fyll ',),' fill '), (('fylthynesse','filthynes'),'filthiness'),
    ...
    ((' hadden ',' hadde '),' had '),((' hadde;',),' had;'),
        ...
        ((' hee ',),' he '),
            ((' heades',' heddes',' heedis'),' heads'), ((' helide',' heelid'),' healed'), ((' helthe ',),' health '), ((' hearde',' herden',' herde',' herd'),' heard'),((' herynge',' hearyng',' heryng',' hearinge',' heringe',' hering'),' hearing'),((' heareth',' herith'),' hears'),((' heare',' heere'),' hear'),
                ...
                ((' heyre ',' heire '),' heir '),
                ((' hertis',' hertes',' heartes'),' hearts'),((' herte ',' hert '),' heart '),((' herte,',' hert,'),' heart,'),((' herte.',),' heart.'), ((' heate',' heete'),' heat'), ((' heauens',' heuenes'),' heavens'), ((' heauen',' heuene',' heven'),' heaven'),
    ...
    ((' keperis',),' keepers'),((' keepeth ',' kepith ',' kepeth '),' keepeth/keeps '),((' keepe ',' kepe '),' keep '),
        ...
        ((' killiden',' kylled',' kyllid'),' killed'),((' kyll.',),' kill.'),
            ((' kyndes',' kindes'),' kinds'),((' kynde',),' kind'), (('kingdome','kyngdoom','kyngdome','kyngdom'),'kingdom'), ((' kynges',' kyngis',' kinges'),' kings'),((' kynge ',' kyng '),' king '),((' kynge,',' kyng,'),' king,'),((' kynge.',' kyng.'),' king.'), ((' kynysman',' kynesman',' kynsman'),' kinsman'),((' kynnysmen',),' kinsmen'), ((' kynne',),' kin'),((' kyn,',),' kin,'),
            ((' kiste ',' kyssed '),' kissed '),
    ...
    (('Sabbathes',),'Sabbaths'),((' sabatys',),' sabbaths'),(('Sabboth','Saboth'),'Sabbath'),((' sabat',' saboth'),' sabbath'),
        ...
        ((' sealeth',),' sealeth/seals'), (('seesyde ' ,'seeside ',),'seaside '), ((' seete ',' seet ',' seate '),' seat '),
            ((' secounde ',' seconde '),' second '),
            ((' seynge',' seinge',' seyng'),' seeing'),(('Se ',),'See '),((' seiy ',' se '),' see '),((' se.',),' see.'), ((' seede ',' sede '),' seed '), ((' sekynge',),' seeking'),((' seken ',' seeke ',' seke '),' seek '), ((' semeth',),' seemeth/seems'),((' semen ',' seeme ',' seme '),' seem '), ((' seyn ',' seene ',' sene '),' seen '),((' seyn,',' seene,',' sene,'),' seen,'),
            ...
            ((' sermoun',),' sermon'),
                (('Serue ',),'Serve '),((' serue ',),' serve '), (('seruauntis','seruauntes','servauntes','seruants','servantes'),'servants'),((' seruaunt',' servaunt',' seruant',' seruaut',' servaut'),' servant'), ((' seruice ',' seruyce '),' service '),
            ((' sette ',),' set '),
    ...
    (('tabernaclis',),'tabernacles/tents'),
            ...
            ((' talke ',),' talk '),((' talke.',),' talk.'), ((' talkinge ',),' talking '),
        ...
        (('ttasfigured',),'transfigured'), ((' trauelid',),' travelled'),
            (('treasurie','tresorie'),'treasury'), ((' tre ',),' tree '),((' tre,',),' tree,'),
    ...
    ((' wayte ',),' wait '),((' waite,',),' wait,'),
        (('walkynge','walkinge'),'walking'),((' walkid',),' walked'),((' walke ',),' walk '),((' walke,',),' walk,'),
             ((' warres',),' wars'),((' warre ',),' war '), ((' warme,',),' warm,'),
...
GERMAN_WORD_MAP = (
    ('Aber','But'),(' aber',' but'),
        (' alle ',' all '),(' alle,',' all,'),(' allen ',' all '), ('allmächtige','almighty'),
            ('Also ','So '),
            ('Altar ','altar '), (' alten ',' old '),
        ('Angesicht ','face '),
        ('Am ','At_the '),(' am ',' in/at/on_the '),
        (' an ',' at '), (' anbeten',' worship'), (' andern ',' change '), ('Anfang','beginning'), ('Antwort','answer'),
        ('Arche ','ark '),
        ('Asche ','ash '),
        (' außen ',' outside '), (' auch ',' also '),(' auch.',' also.'), (' aus ',' out of '), (' auf ',' on '),(' auf,',' on,'), (' aufs ',' onto '),
    ('Ägypten','Egypt'),
    ...
    ('Da ','So '),(' da ',' there '),
        ...
        (' dein ',' your '),(' deine ',' your '),
            ...
            (' des ',' the '),
        (' dich ',' you/yourself '),
            ('Die ','The '),(' die ',' the '), ('Dienst','service'), ('Dies ','This/These '), ('Diese ','This/These '),(' diese:',' this/these:'), (' diesem ',' this_one '),
            (' dir ',' you '),
        (' doch ',' though/but '),
    ...
    (' ehe ',' before '),
        ...
        ('Er ','He '),(' er ',' he '), (' erschien',' appeared'),  (' ersterben',' die'),   (' erwürget',' strangled'),
    ...
    (' fast ',' nearly '),
        ...
        (' folgen ',' follow/obey '),
        ('Freunde','friends'),
        ('Füßen ','feet '), (' führest',' lead'), (' für ',' for '),
    (' gab,',' gave,'), ('Gajus','Gaius'), (' gar ',' even '),
        (' geben ',' give '), (' gebe ',' give '), (' geboten ',' offered '),
            (' gedachte',' thought'),
            (' gehe ',' go '),(' gehen',' go'),
            ('Geist','spirit'),
            ('gekehret','swept'),
        ...
        ('Gottes','God’s'),('GOtt','God'),('Götter','gods'),
        ('Grabe ','grave '), (' groß ',' large '), (' große ',' large '),(' großen',' large'),(' großes',' large'),
        (' gut ',' good '),('Gutes ','Goodness '), ('Gütern ','goods '),
    ('Habe ','goods '), (' habe ',' have '),(' habe,',' have,'),(' habe;',' have;'), ('haben','have'), (' habt',' have'),
            ('halsstarrig','stubborn'), (' halten ',' hold '),
            ('Händen','hands'),
            (' harre ',' wait '),
            (' hat ',' has '),(' hat,',' has,'),(' hat:',' has:'), (' hatte ',' had '),
            ('Hauses','houses'),('Hause ','house '), ('Haut ','skin '),
        (' hebräisch ',' hebrew '),
            (' heilig',' holy'), (' heißt',' is_called'),
            ('heller ','brighter '),
            ('HErr ','LORD '),('HErrn ','LORD '), (' hervor',' out'),
        (' hie ',' here '), (' hieß ',' was_called '), (' hießet ',' is_called '),
            ('Himmel','heaven'),
            (' hin ',' there '),(' hin,',' there,'), (' hinab ',' down '),(' hinauf ',' up '),
        ('Hölle','hell'),
            (' höre',' listen'),
    ('Ich ','I '),(' ich ',' I '),
    ...
    (' kamen ',' came '), (' kam ',' came '), (' kaufen ',' buy '),
        ...
        ('Kinder ','children '),
        ('Kleider ','clothes '),
        ('Komm ','Come '),(' komme ',' come '),(' kommen ',' coming '),
        (' konnten ',' could '),
    ...
    ('Lande','land'), (' lang,',' long,'),(' länger ',' longer '),
        ...
        ('Lebens','life'),('Leben ','life '),(' leben',' life'), (' lebet',' lives'),
            ...
            ('Leib ','body '),
        ('Licht','light'),
            ('Lieben ','loved (one) '), (' liebhabe ',' love '), ('Liebe ','love '),
                (' ließ ',' let '),(' ließen ',' leave/let '), (' liegen',' lie/lay'), (' lieten',' lead'),
            ('Lippen ','lips '),
            ('List ','cunning '),
    (' mache ',' make '),(' machen ',' make '),(' machten',' make'),(' machte ',' made '),
            (' mag ',' like '), ('Männer ','men '), ('Manne ','man '), ('Mann ','man '),('Mann.','man.'), ('Märkte ','marketplaces '),
        (' mecum ',' with_me '),
    ...
    (' sagen',' say'), (' sagt',' says'),
        ...
        ('Schande ','shame '),
                ...
                (' schwebete ',' floated '),
        (' sehe ',' see '), (' sehen',' see'),
            ('Sei ','Be '),(' sei ',' be '), (' sein ',' his '),(' seine ',' his '),(' seinen ',' his '),(' seiner ',' his '), (' seit ',' since '),
        ...
        ('Städte ','cities '), ('Stadt ','city '),
            ('Stamms ','tribe '), (' starb ',' died '), ('Staube ','dust '),('Staub ','dust '),
            (' stehet',' stands'),
                (' steigen',' climb'),
                (' sterbe',' die'),
            (' stille ',' silence '),
    ...
    ('Vaterland','fatherland/homeland'), ('Vater ','father '),('Väter ','fathers '),
        ('vergeben','forgive'), ('verkündigte','announced'), ('verlässest','leave'), ('verloren','lost'), ('versammelt','gathered'), ('versöhnen','reconcile'), ('verständig','sensible'),
    ...
    ('Wahrheit','truth'),
        ...
        (' wider ',' against '), ('Wie ','How '),(' wie ',' like '), (' wieder',' again'), ('Wind ','wind '),
            (' wird ',' becomes '),
            ('Wisset ','Know '),(' wisset ',' know '),
        ('Wo ','Where '),
            (' wollte',' wanted'),
            ('Worten','words'),
        (' wurden ',' became '), ('Wurzel ','root '), (' wüst ',' wild '), (' wußte ',' knew '),
    ('Yahre','years'),
...
LATIN_WORD_MAP = (
    ...
    ('dæmonia','demons'), (' daret',' would_give'),
        (' de ',' about '),
            ...
            (' deserto ',' desert '),
            ('Deum','God'),('Deus','God'),
        (' dices:',' you_say:'), (' dicit:',' he_says:'), ('dicitur','it_is_said'),
    ...
    (' eam ',' her '),
        ...
        (' eos',' them'),
        (' epulis ',' food '),
        (' eradicatæ',' eradicated'), (' erant ',' they_were '), (' erat',' was'),
    ...
    (' radix ',' root '),
        (' rectas ',' correct '),
            ...
            (' reus ',' guilty '),
        (' rursum',' again'),
    (' salvabit',' will_save'), (' sanctum',' holy'), (' sanguinis',' blood'), (' sapientiam',' wisdom'),
        ...
        ('Si ','When/But_if '),(' si ',' when/but_if '),
            ...
            (' sidera ',' stars '),
            (' sine ',' without '),
            (' sit,',' let_it_be,'),(' sit:',' let_it_be:'), (' situ ',' situation '),
            (' sive ',' if/or '),
        (' stare',' to_stand'),
            (' stella ',' star '),
